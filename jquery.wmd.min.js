...
postProcessing();};if(isImage){if(typeof wmd_options.imageDialogText=='function'){wmd_options.imageDialogText(makeLinkMarkdown);}else{util.prompt(wmd_options.imageDialogText,wmd_options.imageDefaultText,makeLinkMarkdown);}}
else{if(typeof wmd_options.linkDialogText=='function'){wmd_options.linkDialogText(makeLinkMarkdown);}else{util.prompt(wmd_options.linkDialogText,wmd_options.linkDefaultText,makeLinkMarkdown);}}
...
chunk.endTag="\n";while(len--){chunk.endTag+=headerChar;}}};command.doHorizontalRule=function(chunk,postProcessing,useDefaultText){chunk.startTag="----------\n";chunk.selection="";chunk.addBlankLines(2,1,true);};command.doInlineMath=function(chunk,postProcessing,useDefaultText){chunk.startTag="\\(";chunk.endTag="\\)";};command.doBlockMath=function(chunk,postProcessing,useDefaultText){chunk.startTag="\\[";chunk.endTag="\\]";chunk.addBlankLines(1,1);};};})();function setup_wmd(options){return new WMDEditor(options);}
...
var _FindLinks=function(text){text=text.replace(/(^|[^<])(https?:[^'">\s]+)($|[^>])/,"$1<$2>$3");text=text.replace(/(^|\s)/,"<mailto:$2>");return text;}
